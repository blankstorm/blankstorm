...
import { locales, settings, _mp, screenshots, current } from './index.js';
import { CelestialBody, Items, Tech, Ship, Player } from '../core/index.js';
	...
	if (current instanceof Level && player.data() instanceof Player) {
		...
		for (let [id, ship] of Ship.generic) {
			...
			const requires = Object.entries(ship.requires).reduce(
				(result, [id, tech]) =>
					`${result}<br>${tech == 0 ? `Incompatible with ${locales.text(`tech.${id}.name`)}` : `${locales.text(`tech.${id}.name`)}: ${player.data().tech[id]}/${tech}`}`,
				''
			);
			ship_ui[id]
				.find('.add tool-tip')
		...
		for (let waypoint of scene.waypoints) {
