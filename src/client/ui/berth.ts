...
export class BerthUI extends HTMLDivElement {
	constructor(berth: Berth, context: UIContext) {
		...
		$(`<p></p>`).addClass('active').text('Building: ' + locales.text(`entity.${berth.productionID}.name`)).appendTo(this);
		const select = $<HTMLSelectElement>('<select></select>').appendTo(this);
		for(const type of shipTypes){
			$(`<option></option>`).attr('value', type).text(locales.text(`entity.${type}.name`)).appendTo(select);
		}
